...
set(gcf,'userdata',struct('icomp',chanorcomp, 'compproj', compproj,'EEG',EEG, 'eegplot',@eegplot));
set(herpim,'hittest','on','ButtonDownFcn','tmp = get(gcf,''userdata''); tmp.time = getpointerpos; tmp.time = round(tmp.time(2)); tmp.figh = findobj(''Name'', [''Effect of removing component #'' num2str(tmp.icomp) '' Black = before rejection; red = after rejection'']); if isempty(tmp.figh); tmp.eegplot( tmp.EEG.data(tmp.EEG.icachansind,:,:), ''time'',tmp.time,''srate'', tmp.EEG.srate, ''title'', [''Effect of removing component #'' num2str(tmp.icomp) '' Black = before rejection; red = after rejection''], ''limits'', [tmp.EEG.xmin tmp.EEG.xmax]*1000, ''data2'', tmp.compproj); else tmp.EPosition = findobj(''tag'',''EPosition'',''parent'',tmp.figh); set(tmp.EPosition, ''String'', num2str(tmp.time)); tmp.eegplot(''drawp'',0,[],tmp.figh);end; clear tmp' )

%%% open a component time-course plot when clicking on topoplot
children_clickthrough(htopo)
set(htopo,'hittest','on','ButtonDownFcn',['tmp = get(gcf,''userdata''); tmpEEG = pop_subcomp(EEG, [' num2str(component_keep) '],0 );tmpdata = eeg_getdatact(tmpEEG, ''component'', 1); tmp.scale = range(tmpdata(:));tmp.eegplot(tmpdata,''srate'',tmpEEG.srate, ''title'', [''Scroll component #'' num2str(tmp.icomp) '' activities''],''limits'', [EEG.xmin EEG.xmax]*1000); tmp.figh = gcf; tmp.ESpacing = findobj(''tag'',''ESpacing'',''parent'',tmp.figh); set(tmp.ESpacing, ''String'', num2str(tmp.scale)); tmp.eegplot(''draws'',0,[],tmp.figh); clear tmp'])

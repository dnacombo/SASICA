...
set(gcf,'userdata',struct('icomp',chanorcomp, 'compproj', compproj,'EEG',EEG, 'eegplot',@eegplot, 'getpointerpos', @getpointerpos,'pop_subcomp',@pop_subcomp,'eeg_getdatact',@eeg_getdatact));
set(herpim,'hittest','on','ButtonDownFcn','tmp = get(gcf,''userdata''); tmp.time = tmp.getpointerpos; tmp.time = round(tmp.time(2)); tmp.figh = findobj(''Name'', [''Effect of removing component #'' num2str(tmp.icomp) '' Black = before rejection; red = after rejection'']); if isempty(tmp.figh); tmp.eegplot( tmp.EEG.data(tmp.EEG.icachansind,:,:), ''time'',tmp.time,''srate'', tmp.EEG.srate, ''title'', [''Effect of removing component #'' num2str(tmp.icomp) '' Black = before rejection; red = after rejection''], ''limits'', [tmp.EEG.xmin tmp.EEG.xmax]*1000, ''data2'', tmp.compproj); else tmp.EPosition = findobj(''tag'',''EPosition'',''parent'',tmp.figh); set(tmp.EPosition, ''String'', num2str(tmp.time)); tmp.eegplot(''drawp'',0,[],tmp.figh);end; clear tmp' )

%%% open a component time-course plot when clicking on topoplot
children_clickthrough(htopo)
set(htopo,'hittest','on','ButtonDownFcn',['tmp = get(gcf,''userdata''); tmpEEG = tmp.pop_subcomp(tmp.EEG, [' num2str(component_keep) '],0 );tmpdata = tmp.eeg_getdatact(tmpEEG, ''component'', [' num2str(chanorcomp) ']); tmp.scale = range(tmpdata(:));tmp.eegplot(tmpdata,''srate'',tmpEEG.srate, ''title'', [''Scroll component #'' num2str(tmp.icomp) '' activities''],''limits'', [tmp.EEG.xmin tmp.EEG.xmax]*1000); tmp.figh = gcf; tmp.ESpacing = findobj(''tag'',''ESpacing'',''parent'',tmp.figh); set(tmp.ESpacing, ''String'', num2str(tmp.scale)); tmp.eegplot(''draws'',0,[],tmp.figh); clear tmp'])
...
if ishandle(winhandle)
    ...
    % OK button
    % ---------
    command = ['tmpEEG = get(findobj(''-regexp'',''name'', ''SASICA 1$''),''userdata'');tmpstatus = get( findobj(''parent'', gcbf, ''tag'', ''rejstatus''), ''userdata'');' ...
        'tmpEEG.reject.gcompreject(' num2str(chanorcomp) ') = tmpstatus;'...
        'set(findobj(''-regexp'',''name'', ''SASICA 1$''),''userdata'',tmpEEG);clear tmpEEG;'];
    if winhandle ~= 0
        command = [ command ...
            sprintf('if tmpstatus set(gcbo, ''backgroundcolor'', %s); else set(gcbo, ''backgroundcolor'', %s); end;', ...
            COLREJ, COLACC) ...
            ['obj = findobj(''-regexp'',''name'',''pop_selectcomps.* -- SASICA''); if isempty(obj); obj = [];else obj = findobj(obj,''tag'',''comp' num2str(chanorcomp) ''');end;'] ...
